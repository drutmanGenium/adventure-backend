...
- **`src/index.ts`** -- Express application setup. Configures CORS (origin from `FRONTEND_URL` env var, defaults to `http://localhost:3000`), JSON body parsing, route mounting under `/api/*`, a health check endpoint, a version endpoint that reads the application version from `package.json` at startup, and a 404 fallback handler. The server listens on `PORT` (default 3001). At module load time, `package.json` is read synchronously from disk using `readFileSync` and parsed to extract the `version` field.
...
**Startup-time package.json reading.** The version endpoint reads `package.json` once at module load time rather than on each request, using `readFileSync` with a path relative to `__dirname`:
...
| Method | Path           | Description                                                        |
|--------|----------------|--------------------------------------------------------------------|
| GET    | /api/version   | Returns `{ version, timestamp }` where `version` is read from `package.json`. |

The version endpoint reads the `version` field from `package.json` (parsed once at startup) and combines it with the current ISO timestamp:
...
- **Version read from package.json.** The `/api/version` endpoint derives its value from `package.json`, which is read synchronously at startup via `readFileSync`. The version string reflects whatever is in `package.json` at the time the server process starts -- it will not pick up changes to `package.json` without a restart.
