...
- **`src/index.ts`** -- Express application setup. Configures CORS (origin from `FRONTEND_URL` env var, defaults to `http://localhost:3000`), JSON body parsing, route mounting under `/api/*`, a health check endpoint, a ping endpoint, and a 404 fallback handler. The server listens on `PORT` (default 3001).
...
| GET    | /api/ping      | Returns `{ pong: true, timestamp }`. Lightweight endpoint for connectivity checks. |

Both endpoints are defined inline in `src/index.ts` (not in separate route files) and return a current ISO timestamp:
