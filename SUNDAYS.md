...
- **`src/index.ts`** -- Express application setup. Configures CORS (origin from `FRONTEND_URL` env var, defaults to `http://localhost:3000`), JSON body parsing, route mounting under `/api/*`, two health check endpoints (`/api/health` and `/healthz`), and a 404 fallback handler. The server listens on `PORT` (default 3001).
...
The API exposes two health check endpoints. `/healthz` is a Kubernetes-style liveness probe at the top level, while `/api/health` lives within the API namespace. Both return the same response shape:
...
|--------|----------------|--------------------------------------------------------------------------|
| GET    | /healthz       | Kubernetes-style liveness probe. Returns `{ status: "ok", timestamp }`.  |
| GET    | /api/health    | API-namespaced health check. Returns `{ status: "ok", timestamp }`.      |
