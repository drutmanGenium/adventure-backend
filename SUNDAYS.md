...
- **`src/index.ts`** -- Express application setup. Configures CORS (origin from `FRONTEND_URL` env var, defaults to `http://localhost:3000`), JSON body parsing, route mounting under `/api/*`, two health check endpoints (`/api/health` and `/healthy`), and a 404 fallback handler. The server listens on `PORT` (default 3001). The `/healthy` endpoint is a root-level health check intended for external monitoring and availability probes, while `/api/health` serves the same purpose under the API prefix.
...
Two health check endpoints are available. The `/healthy` endpoint sits at the application root for use by external monitoring tools, load balancers, and availability probes. The `/api/health` endpoint provides the same check under the API prefix for consistency with other API consumers. Both return an identical response:
...
|--------|----------------|-----------------------------------------------------------------------------|
| GET    | /healthy       | Root-level health check for monitoring and availability probes. Returns `{ status: "ok", timestamp }`. |
| GET    | /api/health    | API-prefixed health check. Returns `{ status: "ok", timestamp }`.           |
