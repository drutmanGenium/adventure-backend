...
- **`src/index.ts`** -- Express application setup. Configures CORS (origin from `FRONTEND_URL` env var, defaults to `http://localhost:3000`), JSON body parsing, route mounting under `/api/*`, a health check endpoint, an uptime endpoint (records server start time at module load and reports `process.uptime()` in seconds), and a 404 fallback handler. The server listens on `PORT` (default 3001).
...
| Method | Path           | Description                                                        |
|--------|----------------|--------------------------------------------------------------------|
| GET    | /api/uptime    | Returns `{ uptimeSeconds, startedAt }` with server uptime info.   |

The uptime endpoint is defined inline in `src/index.ts`. It captures the server start time in a module-level `SERVER_STARTED_AT` constant (a `Date` created when the module loads) and reports `process.uptime()` as seconds since the Node.js process started:
