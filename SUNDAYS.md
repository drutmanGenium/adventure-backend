...
- **`src/index.ts`** -- Express application setup. Configures CORS (origin from `FRONTEND_URL` env var, defaults to `http://localhost:3000`), JSON body parsing, route mounting under `/api/*`, a health check endpoint, an uptime endpoint that reports server uptime in seconds and the ISO timestamp when the process started, and a 404 fallback handler. The server listens on `PORT` (default 3001). A module-level `SERVER_STARTED_AT` constant captures the `Date` at import time for the uptime response.
...
**Module-level timestamp for uptime tracking.** The server start time is captured once at module load and used by the uptime endpoint to report both elapsed seconds and the absolute start time:
...
| Method | Path           | Description                                                                 |
|--------|----------------|-----------------------------------------------------------------------------|
| GET    | /api/uptime    | Returns `{ uptimeSeconds, startedAt }` with process uptime and start time.  |

The `uptimeSeconds` value comes from `process.uptime()` (seconds since the Node.js process started). The `startedAt` value is the ISO 8601 timestamp captured when the module was first loaded. Example response:

```json
{
  "uptimeSeconds": 3661.42,
