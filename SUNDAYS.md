...
- **`src/index.ts`** -- Express application setup. Configures CORS (origin from `FRONTEND_URL` env var, defaults to `http://localhost:3005`), JSON body parsing, route mounting under `/api/*`, a health check endpoint, and a 404 fallback handler. The server listens on `PORT` (default 3001).
...
| FRONTEND_URL  | http://localhost:3005     | Allowed CORS origin for the frontend   |
